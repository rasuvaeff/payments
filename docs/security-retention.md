...
| Revoked claims cannot finalise | `claim()` returns a `WebhookClaimToken` and both finalisers require it back; on a mismatch they are no-ops. A worker whose lease expired and whose claim was taken over cannot `complete()` the new owner's attempt or `release()` its live claim — the latter would open a third concurrent delivery |
